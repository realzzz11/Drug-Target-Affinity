...
class BACPI(nn.Module):
    def __init__(self, task, n_atom, params):
        ...
        # 新增线性层，将amino vector的维度从1024映射到latent_dim
        self.amino_proj = nn.Linear(1024, latent_dim)
    ...
    def bidirectional_attention_prediction(self, atoms_vector, fps, amino_vector):
        ...
        for i in range(self.bidat_num):
            ...
            atoms_att = F.softmax(self.biatt_c[i](atoms_tmp).view(b, -1), dim=1)
            amino_att = F.softmax(self.biatt_p[i](amino_tmp).view(b, -1), dim=1)
    ...
    def forward(self, atoms, atoms_mask, adjacency, amino, amino_mask, fps):
        ...
        amino_vector = amino
        print("amino_vector:", amino_vector.shape) 
