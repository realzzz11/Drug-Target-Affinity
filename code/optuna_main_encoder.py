...
def train_eval(model, task, data_train, data_dev=None, data_test=None, device=None, params=None, trial=None):
    ...
    """
    writer = SummaryWriter(log_dir=os.path.join('optuna_encoder_decoder_runs', f'experiment_{trial.number}'))
    ...
    for epoch in range(80):
        ...
        if data_dev is not None:
            if task == 'affinity':
                ...
                if auc_dev > best_res:
                    best_res = auc_dev
                    torch.save(model, os.path.join('./optuna_encoder_decoder_checkpoint', f'best_model_interaction_{trial.number}.pth'))
...
if __name__ == '__main__':
    ...
    # 保存最终的最佳模型
    torch.save(best_model.state_dict(), './optuna_encoder_decoder_checkpoint/best_model.pth')