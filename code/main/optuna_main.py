...
from dynamic_model import BACPI
from utils import *
...
def objective(trial):
    ...
    # 打印当前试验的超参数组合到txt文件
    with open("./output_dynamicGAT/optuna_dynamicGAT_trial_log.txt", "a") as f:
    ...
    data_dir = os.path.join('./datasets', params.task, params.dataset) 
    # 加载数据
    print('Load data...')
...
def train_eval(model, task, data_train, data_dev=None, data_test=None, device=None, params=None, trial=None):
    ...
    """
    writer = SummaryWriter(log_dir=os.path.join('optuna_dynamicGAT_runs', f'experiment_{trial.number}'))
    ...
    for epoch in range(80):
        ...
        if data_dev is not None:
            if task == 'affinity':
                ...
                if auc_dev > best_res:
                    best_res = auc_dev
                    torch.save(model, os.path.join('./optuna_checkpoint', f'best_model_interaction_{trial.number}.pth'))
...
if __name__ == '__main__':
    ...
    data_dir = os.path.join('./datasets', task, dataset)
    ...
    # 保存最终的最佳模型
    torch.save(best_model.state_dict(), './optuna_dynamicGAT_checkpoint/best_model.pth')