...
class Decoder(nn.Module):
    def __init__(self, embed_dim, hid_dim, n_layers, n_heads, pf_dim, decoder_layer, self_attention,
        ...
        # self.hid_transform = nn.Linear(hid_dim, embed_dim)
    ...
    def forward(self, target_features, source_features, target_mask=None, source_mask=None):
        ...
        """
        # target_features_hid = self.hid_transform(target_features) # [batch_size, num_atoms, embed_dim]
        # logging.info(f"target_features shape before feature_transform: {target_features_hid.shape}")
        target_features = self.dropout(self.feature_transform(target_features)) # [batch_size, num_atoms, hid_dim]
...
class BACPI(nn.Module):
    def __init__(self, task, n_atom, n_amino, params, device):
        ...
        # 解包参数
        comp_dim, prot_dim, gat_dim, num_head, dropout, alpha, window, layer_cnn, hid_dim, layer_out = \
            params.comp_dim, params.prot_dim, params.gat_dim, params.num_head, params.dropout, params.alpha,\
            params.window, params.layer_cnn, params.hid_dim, params.layer_out
        ...
        self.W_comp = nn.Linear(comp_dim, hid_dim)
        ...
        self.W_prot = nn.Linear(prot_dim, hid_dim)

        # Bidirectional encoder-decoder layers
        self.compound_encoder = Encoder(hid_dim, hid_dim, n_layers=2, kernel_size=3, dropout=dropout, device=device)
        self.compound_decoder = Decoder(hid_dim, hid_dim, n_layers=2, n_heads=4, pf_dim=hid_dim * 2, 
        ...
        self.protein_encoder = Encoder(hid_dim, hid_dim, n_layers=2, kernel_size=3, dropout=dropout, device=device)
        self.protein_decoder = Decoder(hid_dim, hid_dim, n_layers=2, n_heads=4, pf_dim=hid_dim * 2, 
                                    decoder_layer=DecoderLayer, self_attention=SelfAttention, 
                                    positionwise_feedforward=PositionwiseFeedforward, dropout=dropout, device=device) 
        
        self.fp0 = nn.Parameter(torch.empty(size=(1024, hid_dim)))
        nn.init.xavier_uniform_(self.fp0, gain=1.414)
        self.fp1 = nn.Parameter(torch.empty(size=(hid_dim, hid_dim)))
        nn.init.xavier_uniform_(self.fp1, gain=1.414)

        # Bidirectional attention mechanism
        self.bidat_num = 4
        # self.U = nn.ParameterList([nn.Parameter(torch.empty(size=(hid_dim, hid_dim))) for _ in range(self.bidat_num)])
        self.U = nn.ParameterList([nn.Parameter(torch.empty(size=(hid_dim, hid_dim))) for _ in range(self.bidat_num)])
        for i in range(self.bidat_num):
            nn.init.xavier_uniform_(self.U[i], gain=1.414)

        self.transform_c2p = nn.ModuleList([nn.Linear(hid_dim, hid_dim) for _ in range(self.bidat_num)])
        self.transform_p2c = nn.ModuleList([nn.Linear(hid_dim, hid_dim) for _ in range(self.bidat_num)])
        self.bihidden_c = nn.ModuleList([nn.Linear(hid_dim, hid_dim) for _ in range(self.bidat_num)])
        self.bihidden_p = nn.ModuleList([nn.Linear(hid_dim, hid_dim) for _ in range(self.bidat_num)])
        self.biatt_c = nn.ModuleList([nn.Linear(hid_dim * 2, 1) for _ in range(self.bidat_num)])
        self.biatt_p = nn.ModuleList([nn.Linear(hid_dim * 2, 1) for _ in range(self.bidat_num)])

        self.comb_c = nn.Linear(hid_dim * self.bidat_num, hid_dim)
        self.comb_p = nn.Linear(hid_dim * self.bidat_num, hid_dim)

        if task == 'affinity':
            self.output = nn.Linear(hid_dim * hid_dim * 2, 1)
        elif task == 'interaction':
            self.output = nn.Linear(hid_dim * hid_dim * 2, 2)
        else:
    ...
    def comp_gat(self, atoms, atoms_mask, adj):
        ...
        atoms_vector = F.leaky_relu(self.W_comp(atom_gat_combined), self.alpha) # [batch_size, num_atoms, hid_dim]
    ...
    def prot_cnn(self, amino, amino_mask):
        ...
        amino_vector = F.leaky_relu(self.W_prot(amino_convolved_squeezed), self.alpha) # [batch_size, num_amino, hid_dim]
    ...
    def encoder_decoder(self, atoms_vector, amino_vector):
        ...
        compound_encoded = self.compound_encoder(atoms_vector) # [batch_size, num_atoms, hid_dim]
        protein_encoded = self.protein_encoder(amino_vector)  # [batch_size, num_amino, hid_dim]
        ...
        compound_decoded = self.compound_decoder(compound_encoded, protein_encoded) # [batch_size, num_atoms, hid_dim]
        protein_decoded = self.protein_decoder(protein_encoded, compound_encoded) # [batch_size, num_amino, hid_dim]
        # logging.info(f"compound_decoded shape: {compound_decoded.shape}")
        # logging.info(f"protein_decoded shape: {protein_decoded.shape}")
        # compound_decoded = self.hid_to_hid(compound_decoded)  # [batch_size, num_atoms, hid_dim]
        # compound_decoded = self.hid_to_hid(compound_decoded)  # [batch_size, num_amino, hid_dim]
    ...
    def bidirectional_attention_prediction(self, atoms_vector, atoms_mask, fps, amino_vector, amino_mask):
        ...
        for i in range(self.bidat_num):
            ...
            # logging.info(f"atoms_trans shape (iteration {i}): {atoms_trans.shape}")  # [batch_size, num_atoms, hid_dim]
            amino_trans = torch.matmul(A_masked.transpose(1, 2), torch.tanh(self.transform_c2p[i](atoms_vector))) #[b, num_amino, hid_dim]
            # logging.info(f"amino_trans shape (iteration {i}): {amino_trans.shape}")  # [batch_size, num_amino, hid_dim]

            # atoms_combined_features：通过self.bihidden_c[i]线性变换，得到化合物原始特征和通过蛋白质加权得到的化合物特征
            atoms_combined_features = torch.cat([torch.tanh(self.bihidden_c[i](atoms_vector)), atoms_trans], dim=2) # [b, num_atoms, hid_dim * 2]
            # logging.info(f"atoms_combined_features shape (iteration {i}): {atoms_combined_features.shape}")  # [batch_size, num_atoms, 2 * hid_dim]
            amino_combined_features = torch.cat([torch.tanh(self.bihidden_p[i](amino_vector)), amino_trans], dim=2) # [b, num_amino, hid_dim * 2]
            # logging.info(f"amino_combined_features shape (iteration {i}): {amino_combined_features.shape}")  # [batch_size, num_amino, 2 * hid_dim]
    ...
    def forward(self, atoms, atoms_mask, adjacency, amino, amino_mask, fps):
        ...
        atoms_vector = self.comp_gat(atoms, atoms_mask, adjacency) # [batch_size, num_atoms, hid_dim]
        ...
        amino_vector = self.prot_cnn(amino, amino_mask) # [batch_size, num_amino, hid_dim]
        # logging.info(f"amino_vector shape in forward: {amino_vector.shape}")

        # 3. 使用全局特征转换
        # logging.info(f"fps shape in forward: {fps.shape}")
        transformed_fps_1 = F.leaky_relu(torch.matmul(fps, self.fp0), 0.1) #[batch_size, hid_dim]
        # logging.info(f"transformed_fps_1 shape: {transformed_fps_1.shape}")
        transformed_fps_2 = F.leaky_relu(torch.matmul(transformed_fps_1, self.fp1), 0.1) #[batch_size, hid_dim]
        # logging.info(f"transformed_fps_2 shape: {transformed_fps_2.shape}")

        # 4. 编码和解码化合物与蛋白质特征
        atoms_decoded, amino_decoded = self.encoder_decoder(atoms_vector, amino_vector) # [batch_size, num_atoms, hid_dim] [batch_size, num_amino, hid_dim]
