...
class BACPI(nn.Module):
    ...
    def comp_gat(self, atoms, atoms_mask, adj):
        # 埋め込みレイヤーを通して原子をベクトルに変換
        atoms_vector = self.embedding_layer_atom(atoms)
        
        # DynamicHeadGATLayerを使用して、グラフアテンションを適用
        atoms_vector = self.gat_layers(atoms_vector, adj)
        
        # GATの出力をELUアクティベーションに通す
        atoms_vector = F.elu(self.gat_out(atoms_vector, adj))
        
        # コンポーネントベクトルをLeaky ReLUで変換し、最終的なベクトルを返す
